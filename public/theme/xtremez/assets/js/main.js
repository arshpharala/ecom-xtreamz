...
function fetchProducts(type, limit, url, extraParams) {
...
function getRecentlyViewedVariantIds(limit) {
    if (typeof localStorage === "undefined") return [];

    try {
        var ids = JSON.parse(
            localStorage.getItem("recently_viewed_variant_ids") || "[]",
        );
...
(function ($, w, d) {
    ...
    function initAjaxCarousel($wrap) {
        ...
        var source = ($wrap.data("source") || "").toString();
        ...
        if (source === "recently-viewed") {
            var ids = getRecentlyViewedVariantIds(limit);
            if (!ids.length) {
                $wrap.prev(".heading-section").hide();
                $wrap.hide();
                return;
            }

            extraParams.include_ids = ids;
        }
        ...
        fetchProducts(type, limit, url, extraParams).then(function (products) {
            if (source === "recently-viewed" && !products.length) {
                $wrap.prev(".heading-section").hide();
                $wrap.hide();
                return;
            }
