...
$(document).ready(function () {
  ...
  function updateSizeVariants() {
    ...
    $sizeInputs.each(function () {
      ...
      const match = window.allVariants.find(v => {
        ...
        return Object.entries(targetCombination).every(([attr, val]) => {
  ...
  function addMultipleSizesToCart(sizeQuantities, $button, buyNow = false) {
    ...
    Promise.all(promises)
      .then(responses => {
        const totalQty = sizeQuantities.reduce((sum, item) => sum + item.qty, 0);
        ...
        $button.html('<span class="added-to-cart">Added to Cart</span>');
