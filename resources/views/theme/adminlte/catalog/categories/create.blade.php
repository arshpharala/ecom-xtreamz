            ...
            @foreach (active_locals() as $locale)
              <div class="form-group">
                ...
                <input type="text" name="name[{{ $locale }}]"
                  class="form-control @error("name.$locale") is-invalid @enderror" value="{{ old("name.$locale") }}"
                  required>
                @error("name.$locale")
                  <span class="text-danger">{{ $message }}</span>
                @enderror
