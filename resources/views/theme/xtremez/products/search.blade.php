...
@section('content')
  <section class="product-section py-5">
    <div class="container">
      <div class="text-center mb-4">
        <form class="cc-search-bar w-100">
          <div class="input-group search-group">
            <input type="text" class="form-control search-input fw-bold type-placeholder" value="{{ $searchQuery }}"
              name="q" id="search-input" aria-label="Search" aria-describedby="search-addon" autocomplete="off"
      ...
      <div class="row g-4" id="searched-products">
        ...
        $.ajax({
          ...
          success: function(response) {
            if (response.success && response.data?.products?.length) {
              ...
              $("#searched-products").html(html);

              render_pagination(response.data.pagination);

            } else {
              $("#searched-products").html(`<p class="text-muted">No products found.</p>`);
            }
          },
          error: function() {
            console.error("Failed to load featured products");
            $("#searched-products").html(`<p class="text-danger">Error loading products.</p>`);
          },
        });
      }
